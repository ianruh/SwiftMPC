...
final class RegressionTests: XCTestCase {
    ...
    func testRegression1() {
        do {
            ...
            guard let _ = (poleLength - Cos(angle))
                .taylorExpand(in: angle, about: previousAngle, ofOrder: 1) else
            {
                XCTFail(
                    "I mean, it apparently didn't crash, but the result shouldn't be optional either."
                )
                return
            }
        }
